...
#include "bridge/esp_bridge_config.h"
#include "bridge/esp_bridge_wifi.h"
#include "bridge/esp_bridge_internal.h"

#include "bridge/esp_bridge.h"

#define CONFIG_BRIDGE_DATA_FORWARDING_NETIF_SOFTAP true
#define CONFIG_BRIDGE_EXTERNAL_NETIF_STATION true
...
static const char *TAG = "bridge_custom_wifi";
    ...
    int32_t event_id, void* event_data)
{
    ...
    printf(ARDUHAL_SHORT_LOG_FORMAT(I,"Connected with IP Address: " IPSTR), IP2STR(&event->ip_info.ip), "\r\n");
...
esp_netif_t* esp_bridge_create_station_netif(esp_netif_ip_info_t* ip_info, uint8_t mac[6], bool data_forwarding, bool enable_dhcps, const char* ssid, const char* passwd)
{
    ...
    int mode2 = (int)mode; mode2 |= WIFI_MODE_STA; mode = (wifi_mode_t)mode2;
    ESP_ERROR_CHECK(esp_wifi_set_mode(mode));

    wifi_sta_config_t router_config;
    memcpy((char *)router_config.ssid, ssid, sizeof(router_config.ssid));
    memcpy((char *)router_config.password, passwd, sizeof(router_config.password));
...
esp_netif_t* esp_bridge_create_softap_netif(esp_netif_ip_info_t* ip_info, uint8_t mac[6], bool data_forwarding, bool enable_dhcps)
{
    ...
    esp_wifi_get_mode(&mode);
    int mode2 = (int)mode; mode2 |= WIFI_MODE_AP; mode = (wifi_mode_t)mode2;
    //mode |= WIFI_MODE_AP;
    ESP_ERROR_CHECK(esp_wifi_set_mode(mode));
