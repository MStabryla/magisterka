    ...
    for (int i = 0; i<IP_PORTMAP_MAX; i++) {
        if (!portmap_tab[i].valid) {
            ...
            if (err == ESP_OK) {
                err = nvs_commit(nvs);
                if (err == ESP_OK) {
                    Serial.println("INFO: New portmap table stored.");
                }
            }
            nvs_close(nvs);
                                ...
                                int32_t event_id, void* event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        //printf("INFO| got ip: %s.%s.%s.%s", IP2STR(&event->ip_info.ip));
        ap_connect = true;
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STACONNECTED)
    {
        ...
        //printf("INFO| %d. station connected", connect_count);
    }
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STADISCONNECTED)
    {
        connect_count--;
        ESP_LOGI(TAG,"station disconnected - %d remain", connect_count);
        //printf("INFO| station disconnected - %d remain", connect_count);
    }
}
